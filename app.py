...
def insert_row(data_frame, list_of_dates):
    ...
    for i in range(0, len(list_of_dates)):
        data_frame.loc[i + last_row] = [data_frame.iloc[last_row]['AssetID'], list_of_dates[i],
                                        data_frame.iloc[last_row]['City'], data_frame.iloc[last_row]['Province'],
                                        data_frame.iloc[last_row]['Country'], data_frame.iloc[last_row]['Latitude'],
                                        data_frame.iloc[last_row]['Longitude']]
    return data_frame
...
def data_frame_by_new_dates_added(grouped_input_data_frame, input_range):
    ...
    for name, group in grouped_input_data_frame:
        input_dates = list_of_dates_to_be_added(grouped_input_data_frame.get_group(name), "7:59", input_range[0],
                                                input_range[1])
...
app.layout = html.Div([
    ...
    html.Div(dcc.Graph( # Working on setting up the default graph
        id="mapbox-graph",
...
def update_graph_daily(daily_dropdown_value, table_rows):
    ...
    assets_groups = df_.groupby("AssetID").apply(lambda x: nearest_previous_date(base_time, x['utc_time'])).reset_index(
        name="utc_time")
...
def update_stackbar(table_rows):
    ...
    dates_idle_cars = count_idle_assets_daily_in_a_time_period(
        assets_are_idle_daily_in_a_time_period(grouped_df, dates_list))
...
app.css.append_css({
    "external_url": "https://codepen.io/hfirdaus/pen/KQKZBN.css"
})
